...
A step-by-step, readable version of [`FinSecure-Capstone-Presentation.pptx`](FinSecure-Capstone-Presentation.pptx) — all 18 slides, in the exact order they appear in the deck, with their literal on-slide titles, their images in the same left-to-right / top-to-bottom order as the slide, and a short explanation of what each one is doing. Verified directly against the packaged `.pptx` (slide titles and image positions extracted from the file itself), so this mirrors the deck as built, not a paraphrase of it.

---

## Step 1 — Fortifying B2B Network Security for Regulatory Compliance

*Case Study · Network Security Capstone*

The deck's title slide: *A FinSecure Technologies Inc. Case Study*, credited to Group 3 (Case Studies, Issues & Capstone Project in Network Security, University of Winnipeg, Prof. Victor Balogun, Fall 2024).
...
## Step 3 — FinSecure Technologies Inc.
...
## Step 4 — A Confirmed Breach — and a Compliance Gap
...
## Step 5 — What This Engagement Set Out to Do
...
Three real screenshots, left to right exactly as they sit on the slide: the actual FinSWebApp login page under test, a Nmap scan confirming only ports 80/443 are exposed externally with a valid TLS cert, and an OWASP ZAP scan flagging 12 alerts including a missing CSP header — all run from the group's own Kali Linux workstation against the group's own Azure deployment, not mockups.
...
Four real screenshots from the group's own Azure subscription, arranged as they are on the slide (top-left to bottom-right): `Fin-SecureVnet` (network segmentation), `FinSecureVault` (secrets/key management), Transparent Data Encryption on the primary SQL database, and the 205-rule Azure WAF policy blocking SQL injection, path traversal, and known CVE exploitation attempts.
